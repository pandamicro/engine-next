...
export default class Particles {
  constructor (device, renderer, config) {
    ...
    this.buffers = {
      updateVB: new gfx.VertexBuffer(device, vertexFmt, gfx.USAGE_STATIC, verts, 4),
      updateIB: new gfx.IndexBuffer(device, gfx.INDEX_FMT_UINT8, gfx.USAGE_STATIC, indices, 6),
      indexes: new gfx.VertexBuffer(device, vertexFmt, gfx.USAGE_STATIC, null, this._tw * this._th * 4)
    };
  ...
  initTextures () {
    let tw = this._tw, th = this._th;
    this._statesize = new Float32Array([tw * 3, th * 3]);
    this._quadsize = new Float32Array([tw * 2, th * 2]);
  ...
  initBuffers () {
    var tw = this._tw, th = this._th,
        indexes = new Float32Array(tw * th * 4 * 2),
        i = 0;
    for (var y = 0; y < th; y++) {
      for (var x = 0; x < tw; x++) {
        indexes[i + 0] = x;
    ...
    this.buffers.indexes._numVertices = tw * th * 4;
    this.buffers.indexes.update(0, indexes);
  }
  ...
  getState (framebuffer) {
    ...
    let w = this._statesize[0], h = this._statesize[1],
  ...
  getQuads () {
    ...
    device.setFrameBuffer(framebuffer);
    let w = this._quadsize[0], h = this._quadsize[1],
        rgba = new Uint8Array(w * h * 4);
    device._gl.readPixels(0, 0, w, h, device._gl.RGBA, device._gl.UNSIGNED_BYTE, rgba);
    let quads = [];
    let off1 = 0, off2 = 0;
  ...
  step (dt) {
    ...
    // setup
    device.setViewport(0, 0, this._statesize[0], this._statesize[1]);
    ...
    device.setUniform('statesize', this._statesize);
    ...
    device.setUniform('quadsize', this._quadsize);
    device.setUniform('statesize', this._statesize);
    device.setUniform('sizeScale', this._sizeScale);
    device.setVertexBuffer(0, this.buffers.updateVB);
    device.setIndexBuffer(this.buffers.updateIB);
